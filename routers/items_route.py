...
class GetPopularItems(MethodView):
    ...
    def get(self):
        ...
        data = db.session.query(itemsModel).join(subquery, db.and_(itemsModel.category == subquery.c.category, itemsModel.price == subquery.c.max_price))
        if data:
            return data
        else:
            abort(400,message="Data not found.")
...
class GetspecialItems(MethodView):
    ...
    def get(self):
        ...
        data = db.session.query(itemsModel).join(subquery, db.and_(itemsModel.category == subquery.c.category, itemsModel.price == subquery.c.min_price))
        if data:
            return data
        else:
            abort(400,message="Data not found.")